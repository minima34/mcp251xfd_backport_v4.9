...
#ifdef MY_REGMAP
static int mcp251xfd_regmap_init(struct mcp251xfd_priv *priv);
#else
...
netdev_err(priv->ndev, "sleep?:%d\n", ( mode_req == MCP251XFD_REG_CON_MODE_SLEEP));
	...
	err = my_regmap_read_poll_timeout(priv->map_reg, MCP251XFD_REG_CON, con,
...
mcp251xfd_chip_wait_for_osc_ready(const struct mcp251xfd_priv *priv, u32 osc_reference, u32 osc_mask)
{
	...
	err = my_regmap_read_poll_timeout(priv->map_reg, MCP251XFD_REG_OSC, osc,
...
printk (KERN_NOTICE "dv_oscva => 0x%08x;\n", priv->dv_oscva);

	if(priv->dv_oscva != ( 0x01 << 31))
	{
		//osc = priv->dv_oscva;
		//netdev_err( priv->ndev, "%s(%d) osc setted to dv_oscva => 0x%08x\n", __FUNCTION__, __LINE__, osc);
		printk (KERN_NOTICE "osc setted to dv_oscva => 0x%08x;\n",osc);
	}
...
static int mcp251xfd_regmap_init(struct mcp251xfd_priv *priv)
{
	...
	config.reg_stride = 4;
	config.val_bits = 32;
	config.max_register = 0xffc;
	config.wr_table = &mcp25xxfd_reg_table;
	config.rd_table = &mcp25xxfd_reg_table;
	config.cache_type = REGCACHE_NONE;
    
    config.name = "nocrc";
	//strcpy(config.name, "nocrc");
	config.pad_bits = 0;

	config.read_flag_mask = (__force unsigned long)
		cpu_to_be16(MCP25XXFD_SPI_INSTRUCTION_READ);
	config.write_flag_mask = (__force unsigned long)
    ...
    //        /!\    LUBO   /!\                     
    //
    // poneje pak go bugnah i nemoga da probvam,
    // build-nal sum, nqma nujda ot preflashvane,
    // 1. otvarqsh Terminala kakto sum go ostavil i pishesh: ./copy_mcp251xfd_modula_to_platkata
    // 2. pishesh parolata szbaijie,
    // 3. otvarqsh leviq na tozi tab na terminala kudeto e screena, samo trqbva da rebootnesh che e zabilo zaradi tazi dolu devm_kzalloc funkciq :D koqto neznaeh kakvo pravq
    // 4. logvash se s root v screena i pishesh:
    //
    //  root@helperboard:~# cd /
    //  root@helperboard:/# ./a
    //
    // gledash za greshki, nai-veroqtno shte ima, i zakachete edin osciloskop ili nai-dobre analizator zashtoto ili e neshto harduerno
    // ili tova regmap api ne se polzva taka v tazi versiq na kernela ili vuobshte ne raboti
    //
    // otdelno mojesh sled tova da probvash gore ima edin define v tozi fail
    // #define MY_REGMAP
    //
    // da go zakomentarish, da buildnesh nanovo i posle pak sushtata procedura:
    //
    //  ./build.sh && ./copy_mcp251xfd_modula_to_platkata
    //
    // i na platkata:
    //
    //   cd / && ./a
    ...
    //
    //
    // LOSHO, NE VIKAI NIKOGA :D
    //
    //priv = devm_kzalloc(&priv->spi->dev, sizeof(*priv), GFP_KERNEL);
	
	priv->map_reg = devm_regmap_init_spi(priv->spi,&config);
